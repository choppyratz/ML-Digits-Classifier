...
void NeuralNetwork::NNInit(vector<int> layersInfo) {
    ...
    for (int i = 0; i < layersInfo.size() - 1; i++) {
        //vector<vector<double>> initColumn = vector<vector<double>>(layersInfo[i], { 1.0 });
        NNLayers[i].push_back({ {1.0} });
       // NNErrors.push_back(initColumn);
    }

#ifdef DEBUG
    NeuralNetwork::NNWeights.push_back({ {0.9, 0.3}, {0.2, 0.8}});
    NeuralNetwork::NNWeights.push_back({ {0.3, 0.7}, {0.6, 0.5}});
#else
    for (int i = 0; i < layersInfo.size() - 1; i++) {
    ...
    //NeuralNetwork::NNWeights.push_back({ {0.9, 0.3}, {0.2, 0.8} });
    //NeuralNetwork::NNWeights.push_back({ {0.3, 0.7}, {0.6, 0.5} });
    //NeuralNetwork::NNWeights.push_back({ {0.3, 0.7}, {0.6, 0.5}, {0.33, -0.2} });
    //NeuralNetwork::NNWeights.push_back({ {0.9, 0.3, -0.1}, {0.2, 0.8, -0.05} });
#endif
    NNLayersNotActivated = NNLayers;
    NNOffsetN = NNLayers;
    for (int i = 0; i < NNLayers.size(); i++) {
        for (int j = 0; j < NNLayers[i].size(); j++) {
            NNOffsetN[i][j][0] = 0.2;
        }
    }
    vector<vector<double>> test = NeuralNetwork::initWeights(3, 5);
}
...
vector<vector<double>> NeuralNetwork::multiplicateMatrixOnSigmoidDerivative(vector<vector<double>> weights, vector<vector<double>> prevOutput, vector<vector<double>> currErrors, vector<vector<double>> currOutput) {
    vector<vector<double>> result = vector<vector<double>>(weights.size());
    for (int i = 0; i < weights.size(); i++) {
        result[i] = vector<double>(weights[0].size());
        for (int j = 0; j < result[0].size(); j++) {
            result[i][j] = -learningRate  * currErrors[i][0] * prevOutput[0][j] * (1 / (1 + exp(-currOutput[i][0]))) * (1 - (1 / (1 + exp(-currOutput[i][0]))));
        }
    }
    //for (int i = 0; i < matrix.size(); i++) {
    //    matrix[i][0] *= (1 / (1 + exp(-matrixOutput[i][0]))) * (1 - (1 / (1 + exp(-matrixOutput[i][0]))));
    //}
    return result;
}
...
vector<vector<double>> NeuralNetwork::changeWeights(vector<vector<double>> oldWeghts, vector<vector<double>> newWeghts) {
    vector<vector<double>> result = vector<vector<double>>(oldWeghts.size());
    for (int i = 0; i < oldWeghts.size(); i++) {
        for (int j = 0; j < oldWeghts[0].size(); j++) {
            result[i].push_back(oldWeghts[i][j] - newWeghts[i][j]);
        }
    }
    return result;
}
